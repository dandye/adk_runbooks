#!/usr/bin/env python
"""Run Detection Engineer A2A agent as a standalone server."""
...
from manager.sub_agents.detection_engineer.agent_a2a import DetectionEngineerA2A
...
# Create FastAPI app
app = FastAPI(title="Detection Engineer A2A Agent")
...
async def root():
    ...
    return {
        ...
        "description": "Detection Engineer specializing in creating and tuning detection rules.",
        "instructions": "Send requests for new detection rules or to tune existing ones.",
...
async def shutdown_event():
    ...
    logger.info("Shutting down Detection Engineer agent...")
    await agent.cleanup()
